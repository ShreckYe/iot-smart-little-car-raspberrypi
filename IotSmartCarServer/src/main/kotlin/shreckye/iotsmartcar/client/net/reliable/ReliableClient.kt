...
class ReliableClient(val host: String, val port: Int = ReliableServiceConstants.DEFAULT_PORT) : AutoCloseable {
    ...
    fun capture(reqeust: IotSmartCarProto.CaptureRequest, responseObserver: StreamObserver<IotSmartCarProto.CaptureResponse>) = asyncStub.capture(reqeust, responseObserver)
    fun getPhotoList(request: IotSmartCarProto.GetPhotoListRequest, responseObserver: StreamObserver<IotSmartCarProto.PhotoItem>) = asyncStub.getPhotoList(request, responseObserver)
